...
function parseGS1(gs1) {
    ...
    while (index <= gs1.length) {
        ...
        var init = parseInt(gs1.substring(index, index + 2));
        switch (init) {
            case 1: // 01 -> GTIN
                // GTIN is 14 digits
            ...
            case 11: // 11 -> Production Date
                var y = 2000 + parseInt(gs1.substring(index + 2, index + 4));
                var m = parseInt(gs1.substring(index + 4, index + 6));
                var d = parseInt(gs1.substring(index + 6, index + 8));
                // YYMMDD
